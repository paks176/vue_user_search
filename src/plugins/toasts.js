...
export default {
    install(Vue) {
        Vue.prototype.$toast = {
            ...
            id="${this.toastId}">
            <div class="toast-wrapper d-flex border-radius-8 p-3">
                ...
                <div class="flex-grow-1">
                 ...
                 ${this.action ? `
                    <div class="mt-2 d-flex">
                        ...
                        <button class="rate-instruct-cancel">������</button>
                    </div>` : ''}
                </div>
                <button class="ms-2 btn-close" data-bs-dismiss="${this.toastId}" aria-label="Close"></button>
                    ...
                    if (this.container) {
                        ...
                        if (appNode) {
                            appNode.insertAdjacentHTML('beforeend', '<div class="toast-container"></div>');
                            this.container = document.querySelector('.toast-container');
                            this.container.insertAdjacentHTML('afterbegin', toastLayout);
                            this.DOMElement = document.getElementById(this.toastId);
                        } else {
                            console.error('Проблема при создании контейнера для тостов')
                        }
                    }
                }
